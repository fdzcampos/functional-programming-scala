...
object Anagrams extends AnagramsInterface {
    ...
    def combinations(occurrences: Occurrences): List[Occurrences] = {
        occurrences.foldRight( List[Occurrences](Nil) )( (occ, joined) => joined ++ (for {
            comb <- joined
            i <- 1 to occ._2
        } yield (occ._1, i) :: comb))
    }
     ...
     *
     *  reminder that:   type Occurrences = List[(Char, Int)]
     */
    def subtract(x: Occurrences, y: Occurrences): Occurrences = {
        x.foldLeft( List[(Char, Int)]() )( ( none, ntwo ) =>
            y.filter( elem => elem._1 == ntwo._1 ) match {
                case List() => none :+ (ntwo._1, ntwo._2)
                case (_, ntwo._2) :: tail => none
                case head :: tail => none :+ (head._1, ntwo._2 - head._2)
            }
        )
    }
        ...
        def sentenceAnagramsAux(occs: Occurrences): List[Sentence] = {
            if( occs.isEmpty ) List(List())
                ...
                for{
                    ...
                    w <- dictionaryByOccurrences getOrElse( cmb, Nil)
                    s <- sentenceAnagramsAux(subtract(occs, wordOccurrences(w)))
                    if cmb.nonEmpty
                } yield w :: s
            }
        }
